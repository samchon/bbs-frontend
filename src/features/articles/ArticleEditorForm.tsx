...
import type { AppError, ArticleEditorValues, ArticleUpdateValues } from "../../domain/models";

type ArticleFormValues = ArticleEditorValues;

type CreateProps = {
  mode: "create";
  initialValues: ArticleEditorValues;
  onSubmit: (values: ArticleEditorValues) => Promise<void> | void;
};

type UpdateProps = {
  mode: "update";
  initialValues: ArticleUpdateValues;
  onSubmit: (values: ArticleUpdateValues) => Promise<void> | void;
};

type ArticleEditorFormProps = (CreateProps | UpdateProps) & {
  submitLabel: string;
  busy?: boolean;
  error?: AppError | null;
      ...
      <section className="editor-panel">
        {props.showHeader !== false ? (
          <div className="section-header">
            <div>
              <h2>{props.mode === "create" ? "Write a post" : "Edit post"}</h2>
              <p>
                {props.mode === "create"
                  ? "Fill out the form below and publish it to the board."
                  : "Save your changes to make them the newest version of this post."}
              </p>
            </div>
          </div>
        ) : null}
